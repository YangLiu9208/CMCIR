...
from model.netvladv2 import NetVLAD, NetVLAD_four, GAFN_V2, GAFN_V2_Four, GAFN, GAFN_Four
...
class InputUnitLinguistic_Transformer(nn.Module):
    def __init__(self, vocab_size, wordvec_dim=300, module_dim=512):
        ...
        self.TransformerEncoder_text = TransformerEncoder(embed_dim=module_dim, pos_flag='sincos',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=8)
...
class InputUnitVisual_GST_Transformer_VLAD_all(nn.Module):
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding, question_embedding_dict,question_embedding_do, appearance_dict, motion_dict):
        """
        Args:
            appearance_video_feat: [Tensor] (batch_size, num_clips, num_frames, visual_inp_dim)
            motion_video_feat: [Tensor] (batch_size, num_clips, visual_inp_dim)
        ...
        question_visual_m = self.QMTransformer(question_embedding,visual_embedding_motion.permute(1,0,2))
        question_visual_m_dict = self.QMTransformer(question_embedding_dict,visual_embedding_motion_dict)
        question_visual_m_do = self.QMTransformer(question_embedding_do,visual_embedding_motion_vlad.permute(1,0,2))
        question_visual_a  = self.QATransformer(question_embedding,torch.mean(visual_embedding_appearance.permute(1,2,0,3), 1))
        question_visual_a_dict  = self.QATransformer(question_embedding_dict,visual_embedding_appearance_dict)
        ...
        return torch.mean(torch.cat((question_visual_m,question_visual_a),2),0), torch.mean(torch.cat((question_visual_m_dict,question_visual_a_dict),2),0),\
        torch.mean(torch.cat((question_visual_m_do,question_visual_a_do),2),0), torch.cat((visual_embedding_m,visual_embedding_a),2).permute(1,0,2),\
             torch.cat((visual_embedding_m_dict,visual_embedding_a_dict),2).permute(1,0,2),torch.cat((visual_embedding_m_do,visual_embedding_a_do),2).permute(1,0,2)
...
class OutputUnitOpenEnded_ST(nn.Module):
    def __init__(self, module_dim=512, num_answers=1000):
        ...
        self.question_proj = nn.Linear(18*module_dim, module_dim)
        self.visual_proj = nn.Linear(18*module_dim, module_dim)
...
class VLCIR(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim, word_dim,  vocab, vocab_subject, vocab_relation, vocab_object, question_type):
        ...
        if self.question_type in ['action']:
            ...
            self.linguistic_input_unit = InputUnitLinguistic_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_input_unit_do = InputUnitLinguistic_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_input_unit_dict = InputUnitLinguistic_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_subject_input_unit = InputUnitLinguistic_subject_Transformer(vocab_size=encoder_vocab_subject_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_subject_input_unit_do = InputUnitLinguistic_subject_Transformer(vocab_size=encoder_vocab_subject_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_subject_input_unit_dict = InputUnitLinguistic_subject_Transformer(vocab_size=encoder_vocab_subject_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_relation_input_unit = InputUnitLinguistic_relation_Transformer(vocab_size=encoder_vocab_relation_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_relation_input_unit_do = InputUnitLinguistic_relation_Transformer(vocab_size=encoder_vocab_relation_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_relation_input_unit_dict = InputUnitLinguistic_relation_Transformer(vocab_size=encoder_vocab_relation_size, wordvec_dim=word_dim,module_dim=module_dim)
            self.linguistic_object_input_unit = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim)                                                                                                         
            self.linguistic_object_input_unit_do = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim) 
            self.linguistic_object_input_unit_dict = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim) 
            self.visual_input_unit = InputUnitVisual_GST_Transformer_VLAD_all(motion_dim=motion_dim, appearance_dim=appearance_dim, module_dim=module_dim)
            self.output_unit = OutputUnitOpenEnded_ST(num_answers=self.num_classes)
            self.fusion=SE_Fusion_Three(1024,1024,1024,3).cuda()
            self.fusion_do=SE_Fusion_Three(1024,1024,1024,3).cuda()
            self.fusion_dict=SE_Fusion_Three(1024,1024,1024,3).cuda()
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, question,question_subject, question_relation, question_object, question_len, question_subject_len, question_relation_len, question_object_len, appearance_dict, motion_dict):
        ...
        #         answer_embedding_v_expand, visual_embedding_an_expand, answers_len)
            ...
            question_embedding = self.linguistic_input_unit(question, question_len)
            question_embedding_do = self.linguistic_input_unit_do(question, question_len)
            question_embedding_dict = self.linguistic_input_unit_dict(question, question_len)
            question_embedding_subject = self.linguistic_subject_input_unit(question_subject, question_subject_len)
            question_embedding_subject_do = self.linguistic_subject_input_unit_do(question_subject, question_subject_len)
            question_embedding_subject_dict = self.linguistic_subject_input_unit_dict(question_subject, question_subject_len)
            question_embedding_relation = self.linguistic_relation_input_unit(question_relation, question_relation_len)
            question_embedding_relation_do = self.linguistic_relation_input_unit_do(question_relation, question_relation_len)
            question_embedding_relation_dict = self.linguistic_relation_input_unit_dict(question_relation, question_relation_len)
            # question_embedding_object = self.linguistic_object_input_unit(question, question_len) 
            # question_embedding_object_do = self.linguistic_object_input_unit_do(question, question_len) 

            question_embedding, question_embedding_dict, question_embedding_do, visual_embedding, visual_embedding_dict,visual_embedding_do \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding, question_embedding_dict, question_embedding_do, appearance_dict, motion_dict )
            question_embedding_subject, question_embedding_subject_dict, question_embedding_subject_do, visual_embedding_subject, visual_embedding_dict_subject, visual_embedding_do_subject \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_subject, question_embedding_subject_dict, question_embedding_subject_do, appearance_dict, motion_dict )
            question_embedding_relation, question_embedding_relation_dict, question_embedding_relation_do, visual_embedding_relation, visual_embedding_dict_relation, visual_embedding_do_relation \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_relation, question_embedding_relation_dict, question_embedding_relation_do, appearance_dict, motion_dict )
            # question_embedding_object, question_embedding_object_do, visual_embedding_object, visual_embedding_do_object \
            #     = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_object, question_embedding_object_do, appearance_dict, motion_dict )
            ...
            question_embedding_dict,question_embedding_subject_dict,question_embedding_relation_dict \
                 =self.fusion_dict(question_embedding_dict,question_embedding_subject_dict,question_embedding_relation_dict)
            ...
            question_embedding_all = torch.cat((question_embedding, question_embedding_subject, question_embedding_relation), 1)
            question_embedding_all_dict = torch.cat((question_embedding_dict, question_embedding_subject_dict, question_embedding_relation_dict), 1)
            question_embedding_all_do = torch.cat((question_embedding_do, question_embedding_subject_do, question_embedding_relation_do), 1)

            visual_embedding_all = self.feature_aggregation(question_embedding, question_embedding_subject, question_embedding_relation, \
                    visual_embedding, visual_embedding_subject, visual_embedding_relation)
            visual_embedding_all_dict = self.feature_aggregation(question_embedding_dict, question_embedding_subject_dict, question_embedding_relation_dict, \
                        visual_embedding_dict, visual_embedding_dict_subject, visual_embedding_dict_relation)
            visual_embedding_all_do = self.feature_aggregation(question_embedding_do, question_embedding_subject_do, question_embedding_relation_do, \
                        visual_embedding_do, visual_embedding_do_subject, visual_embedding_do_relation)

            out = self.output_unit(torch.cat((question_embedding_all,question_embedding_all_dict,question_embedding_all_do),1),torch.cat((visual_embedding_all,visual_embedding_all_dict,visual_embedding_all_do),1))
