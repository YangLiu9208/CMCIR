...
from model.netvladv2 import GAFN, GAFN_Four
...
class InputUnitVisual_GST_Transformer_VLAD(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim=512):
        ...
        self.VIMTransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='sincos',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=8)
        self.module_dim = module_dim
        self.net_vlad_motion = NetVLAD(num_clusters=1, dim=module_dim, alpha=1.0)
        self.net_vlad_appearance = NetVLAD_four(num_clusters=1, dim=module_dim, alpha=1.0)
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight)
                nn.init.constant_(m.bias, 0)
...
class InputUnitVisual_GST_Transformer_TGIF_VLAD_count(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim=512):
        ...
        self.VIMTransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='sincos',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=8)         
        self.module_dim = module_dim
        self.net_vlad_motion = NetVLAD_V2(num_clusters=1, dim=module_dim, normalize_input=True, vladv2=False, use_faiss=True)
        self.net_vlad_appearance = NetVLAD_V2(num_clusters=1, dim=module_dim, normalize_input=True, vladv2=False, use_faiss=True)
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight)
                nn.init.constant_(m.bias, 0)
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding, question_embedding_do,appearance_dict, motion_dict):
        ...
        visual_embedding_motion_dict = self.net_vlad_motion(visual_embedding_motion_dict)
        #visual_embedding_motion_dict = self.clip_level_motion_VLAD_proj(visual_embedding_motion_dict)
        visual_embedding_appearance_dict = self.net_vlad_appearance(visual_embedding_appearance_dict)
        #visual_embedding_appearance_dict = self.appearance_feat_VLAD_proj(visual_embedding_appearance_dict)

        question_visual_m = self.QMTransformer(question_embedding,visual_embedding_motion.permute(1,0,2))
        question_visual_m_do = self.QMTransformer(question_embedding_do,visual_embedding_motion_dict.permute(1,0,2))
        question_visual_a  = self.QATransformer(question_embedding,torch.mean(visual_embedding_appearance.permute(1,2,0,3), 1))
        question_visual_a_do  = self.QATransformer(question_embedding_do,visual_embedding_appearance_dict.permute(1,0,2))

        visual_embedding_m = self.VIMTransformer(visual_embedding_motion.permute(1,0,2),question_visual_m)
        visual_embedding_m_do = self.VIMTransformer(visual_embedding_motion_dict.permute(1,0,2),question_visual_m_do)
        visual_embedding_a = self.VIATransformer(torch.mean(visual_embedding_appearance.permute(1,2,0,3), 1),question_visual_a)
        visual_embedding_a_do = self.VIATransformer(visual_embedding_appearance_dict.permute(1,0,2),question_visual_a_do)
...
class InputUnitVisual_GST_Transformer_TGIF_VLAD_action(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim=512):
        ...
        self.VIMTransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='sincos',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=8)        
        self.module_dim = module_dim
        self.net_vlad_motion = NetVLAD_V2_Cluster(num_clusters=1, dim=512, normalize_input=True, vladv2=False, use_faiss=False)
        self.net_vlad_appearance = NetVLAD_V2_Cluster(num_clusters=1, dim=512, normalize_input=True, vladv2=False, use_faiss=False)
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight)
                nn.init.constant_(m.bias, 0)
...
class SE_Fusion_Four(nn.Module):
    def __init__(self,channel_1=128,channel_2=256,channel_3=256,channel_4=256,reduction=4):
        ...
        self.fc_1 = nn.Sequential(
            nn.Linear((channel_1+channel_2+channel_3+channel_4), (channel_1+channel_2+channel_3+channel_4) // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2+channel_3+channel_4) // reduction, channel_1, bias=False),
            nn.Sigmoid()
        )
        self.fc_2 = nn.Sequential(
            nn.Linear((channel_1+channel_2+channel_3+channel_4), (channel_1+channel_2+channel_3+channel_4) // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2+channel_3+channel_4) //reduction, channel_2, bias=False),
            nn.Sigmoid()
        )
        self.fc_3 = nn.Sequential(
            nn.Linear((channel_1+channel_2+channel_3+channel_4), (channel_1+channel_2+channel_3+channel_4) // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2+channel_3+channel_4) //reduction, channel_3, bias=False),
            nn.Sigmoid()
        )
        self.fc_4 = nn.Sequential(
            nn.Linear((channel_1+channel_2+channel_3+channel_4), (channel_1+channel_2+channel_3+channel_4) // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2+channel_3+channel_4) //reduction, channel_4, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, fea1, fea2, fea3, fea4):
        ...
        #edge_index=torch.tensor([[0,1,0,2,0,3],[1,0,2,0,3,0]],dtype=torch.long).cuda()
...
class VLCIR(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim, word_dim,  vocab, vocab_subject, vocab_relation, vocab_object, question_type):
        ...
        if self.question_type in ['action']:
            ...
            self.visual_input_unit = InputUnitVisual_GST_Transformer_TGIF_VLAD_action(motion_dim=motion_dim, appearance_dim=appearance_dim, module_dim=module_dim)
            ...
            self.linguistic_object_input_unit = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim)                                                                                                         
            self.linguistic_object_input_unit_do = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim) 
            self.visual_input_unit = InputUnitVisual_GST_Transformer_TGIF_VLAD_transition(motion_dim=motion_dim, appearance_dim=appearance_dim, module_dim=module_dim)
            ...
            self.linguistic_object_input_unit = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim)                                                                                                         
            self.linguistic_object_input_unit_do = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim) 
            self.visual_input_unit = InputUnitVisual_GST_Transformer_TGIF_VLAD_count(motion_dim=motion_dim, appearance_dim=appearance_dim, module_dim=module_dim)
            ...
            self.linguistic_object_input_unit = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim)                                                                                                         
            self.linguistic_object_input_unit_do = InputUnitLinguistic_object_Transformer(vocab_size=encoder_vocab_object_size, wordvec_dim=word_dim,module_dim=module_dim) 
            self.visual_input_unit = InputUnitVisual_GST_Transformer_VLAD(motion_dim=motion_dim, appearance_dim=appearance_dim, module_dim=module_dim)
