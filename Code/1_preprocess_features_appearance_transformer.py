...
def run_batch_sutd_transformer(cur_batch, model):
    """
    Args:
        cur_batch: treat a video as a batch of images
        model: ResNet model for feature extraction
                ...
                if valid:
                    for clip_id, clip in enumerate(clips):
                        if args.dataset=='sutd-qa':
                            feats = run_batch_sutd_transformer(clip, model)  # (16, 2048)
                        else:
                            feats = run_batch(clip, model)  # (16, 2048)
                        feats = feats.squeeze()
                        clip_feat.append(feats)
                else:
                    clip_feat = np.zeros(shape=(num_clips, 16, 3, 224, 224))
                clip_feat = np.asarray(torch.stack(clip_feat))  # (8, 16, 2048)
                if feat_dset is None:
                    A,B,C, D,E = clip_feat.shape
                    feat_dset = fd.create_dataset('raw_appearance_clips', (dataset_size, A,B,C, D,E),
                                                  dtype=np.float32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', dest='outfile',
                        ...
                        default="data/{}/{}_{}_raw_clips.h5", type=str)
    ...
    # network params
    parser.add_argument('--model', default='resnet101', choices=['resnet101', 'resnext101','ViViT'], type=str)
