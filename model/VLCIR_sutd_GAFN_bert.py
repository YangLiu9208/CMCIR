...
class VLCIR(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim, word_dim,  vocab, vocab_subject, vocab_relation, vocab_object, question_type):
        ...
        if self.question_type in ['action']:
            ...
            self.fusion=SE_Fusion_Four(2*module_dim,2*module_dim,2*module_dim,2*module_dim,4).cuda()
            self.fusion_do=SE_Fusion_Four(2*module_dim,2*module_dim,2*module_dim,2*module_dim,4).cuda()
            self.fusion_visual=SE_Fusion_Four(2*module_dim,2*module_dim,2*module_dim,2*module_dim,4).cuda()
            self.fusion_visual_do=SE_Fusion_Four(2*module_dim,2*module_dim,2*module_dim,2*module_dim,4).cuda()
