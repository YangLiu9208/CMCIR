...
def validate(cfg, model, data, device, write_preds=False):
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            ...
            if cfg.dataset.name == 'sutd-qa':
                ...
                key_word1 = batch_input[-10][:,0].to('cpu') # batch-based questions word
                key_word2 = batch_input[-10][0,:].to('cpu') # batch-based questions word
                for i in range(0,batch_input[-10].size(0)):
                    for j in range(0,batch_input[-10].size(1)):
                        key_word= batch_input[-10][i,j].to('cpu') 
                        key_word = int(key_word)
                        if data.vocab['question_idx_to_token'][key_word] == 'reason': #2369
                            attri_idx.append(i)
                            continue
                        elif data.vocab['question_idx_to_token'][key_word] == 'prevent' or data.vocab['question_idx_to_token'][key_word] == 'avoid' or data.vocab['question_idx_to_token'][key_word] == 'could': #2758
                            intro_idx.append(i)
                            continue
                        elif data.vocab['question_idx_to_token'][key_word] == 'if' or data.vocab['question_idx_to_token'][key_word] == 'still' : #1276
                            counter_idx.append(i)
                            continue
                        elif data.vocab['question_idx_to_token'][key_word] == 'will': #3278
                            forcast_idx.append(i)
                            continue
                        elif data.vocab['question_idx_to_token'][key_word] == 'might':  #1915
    ...
    if not write_preds:
        ...
        if cfg.dataset.name == 'sutd-qa':
            return acc, all_preds, gts, v_ids, q_ids, basic_acc, attri_acc, intro_acc, counter_acc, forcast_acc, reverse_acc,  basic_count, attri_count, intro_count, counter_count, forcast_count, reverse_count 
